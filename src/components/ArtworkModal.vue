    ...
    <div class="artwork-modal" @click.stop>
      ...
      <div v-else-if="artwork" class="artwork-content">
        ...
        <div class="artwork-image">
          ...
          <!-- 圖片切換按鈕 -->
          <div
            v-if="!loading && photoCheckCompleted"
            class="image-toggle-controls"
          >
            <!-- 只有現實照片時才顯示兩個切換按鈕 -->
            <template v-if="hasRealPhoto">
              <button
                @click="
                  () => {
                    if (hasRealPhoto) showRealPhoto = false;
                  }
                "
                class="toggle-btn"
              ...
              <button
                @click="
                  () => {
                    if (hasRealPhoto) showRealPhoto = true;
                  }
                "
                class="toggle-btn"
                :class="{ active: showRealPhoto }"
              >
                {{ $t("artwork.realPhotoView") || "實景對比" }}
              </button>
            </template>
            <!-- 沒有現實照片時只顯示藝術作品標籤 -->
            <template v-else>
        ...
        <div class="artwork-info">
          ...
          <div v-if="isMultipleArtwork" class="multiple-artwork-info">
            <p class="info-text">
              {{
                $t("artwork.multipleInfo") ||
                "This location contains multiple artworks"
              }}
            </p>
          </div>
        </div>
...
const getCurrentImage = computed(() => {
  ...
  return showRealPhoto.value && hasRealPhoto.value
    ? getRealPhotoImage(artwork.value.id)
    : getArtworkImage(artwork.value.id);
});
...
const fetchArtwork = async (artworkId: string | number) => {
  ...
  try {
    ...
    // 查找對應的藝術品
    const foundArtwork = artworksData.find(
      (item: any) => String(item.id) === primaryId
    );

    if (foundArtwork) {
      console.log(
        `Found artwork: ${foundArtwork.title} (ID: ${foundArtwork.id})`
      );
      ...
      console.log(
        `Final state: hasRealPhoto=${hasRealPhoto.value}, photoCheckCompleted=${photoCheckCompleted.value}`
      );
    } else {
